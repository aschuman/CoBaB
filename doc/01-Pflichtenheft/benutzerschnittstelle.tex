...
In den Bildern werden außerdem, falls vorhanden, \glslink{Annotation}{Annotationen} angezeigt. Davon kann eine (mit Klick darauf) ausgewählt werden. Optional kann auch ein eigenes Rechteck gezogen werden. Mit Rechtsklick auf eine \gls{Annotation} erscheint eine Auswahl aller \glslink{Suchverfahren}{Suchalgorithmen}, die für diese \gls{Annotation} in Frage kommen. Der Benutzer kann durch Klicken auf einen Algorithmusnamen mit dem Programm fortfahren.
...
\end{figure}
Nach der Auswahl eines \glslink{Suchverfahren}{Suchverfahrens} kann man Parameter für den \glslink{Suchverfahren}{Suchalgorithmus} festlegen. Außerdem können weitere Datensätze ausgewählt werden, in denen gesucht werden soll.
...
\end{figure}
Im Überprüfungsfenster wird noch einmal die aktuelle Auswahl angezeigt: der gewählte Bild-/Videoausschnitt, die Datensätze, in denen gesucht wird, der \glslink{Suchverfahren}{Suchalgorithmus} und die Parameter. Wenn der Benutzer an diesen Werten nichts mehr ändern will, kann er nun über den Button \enquote{Suche starten} das \gls{Suchverfahren} mit der gezeigten Auswahl starten.
